---
compatibility: Requires the .NET SDK and Microsoft.Windows.CsWin32 0.3.296 or later; Windows COM APIs across .NET 10 and .NET Framework. Cross-assembly extension-block examples require C# 14.
...
The baseline is Microsoft.Windows.CsWin32 0.3.296 or later. Earlier generator
versions are outside this skill's compatibility contract; upgrade before
applying its `IComIID` or CCW guidance.
...
For a manual `IComIID` implementation on `NET`, "stable IID" means a ref to a
constant 16-byte `ReadOnlySpan<byte>` that the compiler can emit as RVA data,
following the pattern generated by CsWin32. The non-`NET` branch cannot use that
pattern; return a ref to an initialized static `Guid` field there. For RVA bytes,
encode the first three GUID components little-endian and the final eight bytes
in display order. Pin both target paths with an exact GUID value test. See
...
6. **Gate .NET 10-only COM features with `#if NET`.** `ComWrappers`-based CCW support is unavailable on .NET Framework. Generated `IComIID` works on both supported runtime families; see [comiid-and-cls.md](comiid-and-cls.md).
...
- Prefer `IID.Get<T>()` when available; it returns a `Guid*` to the type's
  ...
  one synchronous native call. The RVA rule applies only to the `NET` branch of
  a type-owned manual `IComIID` implementation, not to a non-`NET` branch or
  such a call-scoped local.
...
- [comiid-and-cls.md](comiid-and-cls.md) - `IComIID` across .NET 10 and
  .NET Framework, manual IID storage, and CLS compliance.
