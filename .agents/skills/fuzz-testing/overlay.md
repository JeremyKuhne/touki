...
[touki.fuzz/README.md](../../../test/touki.fuzz/README.md) - read it first; this overlay
only binds the names.

## Concrete bindings for the core's placeholders

- **Harness project** (`<root>.fuzz`):
  [touki.fuzz](../../../test/touki.fuzz/touki.fuzz.csproj), namespace `Touki.Fuzz`. It
...
- **Prerequisites**:
  [touki.fuzz/Install-FuzzPrereqs.ps1](../../../test/touki.fuzz/Install-FuzzPrereqs.ps1).
- **Target registration**: the `FUZZ_TARGET` switch in
  [touki.fuzz/Program.cs](../../../test/touki.fuzz/Program.cs); one `<Type>Target.cs`
  per target (e.g.
  [SpanReaderTarget.cs](../../../test/touki.fuzz/SpanReaderTarget.cs)). Current targets:
  `SpanReader`, `SpanWriter`, `RunLength`, `StringSegment`, `ValueStringBuilder`,
  `GlobSpecification`.
- **Invariant exception**:
  [touki.fuzz/FuzzInvariantException.cs](../../../test/touki.fuzz/FuzzInvariantException.cs).
- **Plan and phases**:
