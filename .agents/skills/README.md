...
| [dotnet-file-creation](./dotnet-file-creation/SKILL.md) | creating temporary, persisted, or shared files and directories; `Path.Join` / qualification / deterministic resolution; owner-only files; cross-platform `FileShare`, delete, hidden-file, and mode behavior; atomic write-then-rename publishing | vendored (portable core) + overlay | `windows-acls`, `security-review`, `dotnet-polyfills`, `polyfill-dotnet-api`, `run-tests-on-wsl` (via [overlay](./dotnet-file-creation/overlay.md)) |
...
| [pre-pr-self-review](./pre-pr-self-review/SKILL.md) | self-review checklist plus a read-only agentic review pass before opening or pushing a PR; missing tests, unchecked length sums, empty-span foot-guns, TFM phrasing | vendored (portable core) + overlay | `create-pr`, `address-pr-feedback`, `security-review`, `performance-testing`, `technical-writing`, `polyfill-dotnet-api` (via [overlay](./pre-pr-self-review/overlay.md)) |
| [security-review](./security-review/SKILL.md) | "assess for security vulnerabilities", "do a security review", "check for ReDoS / DoS", "audit untrusted input handling"; any member accepting caller-supplied data, any use of `unsafe` / `Unsafe.*` / `MemoryMarshal.*` / `Marshal.*`, or any BCL API whose name or doc says "unsafe" / "caller must" - abusive-input handling, length / integer overflow, allocation and algorithmic DoS, caller-validated preconditions, argument validation | vendored (portable core) + overlay | `pre-pr-self-review`, `performance-testing`, `fuzz-testing`, `dotnet-file-creation` (via [overlay](./security-review/overlay.md)) |
| [create-pr](./create-pr/SKILL.md) | "make a PR", "prepare a PR", "open a pull request", "push and PR", "publish for review" - **initial** preparation and publish | vendored (portable core) + overlay | `technical-writing`, `pre-pr-self-review`, `address-pr-feedback` |
| [address-pr-feedback](./address-pr-feedback/SKILL.md) | "address the review", "fix the comments", "address Copilot's feedback", "fix the CI failure" - **post-PR** review-cycle work | vendored (portable core) + overlay | `technical-writing`, `create-pr`, `pre-pr-self-review`, `agent-files-review` |
| [agent-files-review](./agent-files-review/SKILL.md) | reviewing/validating agent-customization files (`AGENTS.md`, `*.instructions.md`, `*.prompt.md`, `*.agent.md`, `SKILL.md`, validator, CI workflow); fixing `agent-files.yml` failures | vendored (portable core) + overlay | `manage-skills`, `technical-writing` |
...
| [cswin32-com](./cswin32-com/SKILL.md) | struct-based COM interop with raw pointers, `ComScope`, `IComIID`, `delegate* unmanaged` vtables, COM activation, connection points, CCW bridges, manual COM structs, or cross-assembly COM composition | vendored (portable core) + overlay | `cswin32-interop`, `security-review`, `il-copy-inspection` (via [overlay](./cswin32-com/overlay.md)) |
...
Both touch skill files, but lifecycle work chains into file validation:

- **The catalog lifecycle** - finding, building, reviewing, updating, or retiring
  a skill -> start with `manage-skills`.
- **Validating one agent file** - frontmatter, mirror sync, whitespace, the
  `agent-files.yml` gate -> use `agent-files-review` directly.

Lifecycle work finishes with `agent-files-review`; direct file/schema diagnostics
do not need the lifecycle workflow first.

### `technical-writing` vs domain review skills

`technical-writing` owns the reader-facing artifact after domain facts are settled:

- **Agent customization behavior, frontmatter, mirrors, links** ->
  `agent-files-review`, followed by `technical-writing` only for comprehension.
- **Source-code naming, nesting, and cognitive load** -> `code-comprehension`.
- **PR claims matching the diff and test evidence** -> `pre-pr-self-review`, then
  `technical-writing` for the exact title, body, commit message, or reply.

The commons `user-voice` skill is intentionally not project-vendored. It manages
private personal profiles that belong at user scope, never in this public repo.
...
### `dotnet-file-creation` vs polyfills and security review

These workflows split by the decision being made:

- **Where and how to create or publish a file, resolve a path, or handle platform
  file semantics** -> `dotnet-file-creation`.
- **Whether the selected API exists on `net472` and which package supplies it** ->
  `dotnet-polyfills`, then `polyfill-dotnet-api` only as a last resort.
- **Whether untrusted input can escape containment or reach a privileged file
  operation** -> `security-review`.

The commons `windows-acls` skill is intentionally not vendored: Touki currently
has no `DirectorySecurity` / `FileSecurity` or privileged shared-state surface.
...
The commons `engineering-baseline` skill is intentionally **not vendored here**:
Touki is already an established repository. Its name appears only as an
upstream-owned cross-reference.
