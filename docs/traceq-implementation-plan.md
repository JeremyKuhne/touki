...
**Status: in progress.** Step 1 (relocation + provider/engine seam), the output-contract envelope, and the symbol gate have landed; the contract's budget/ceiling, steering hints and text renderer, the ETLX cache, new providers/verbs, `trim`, and the O1 spike remain.
...
2. **Output contract (§4.4)** as a core type every service returns through. **Envelope landed**: `AnalysisResult<T>` carries a `schemaVersion`, a warnings channel, a steering-hints channel, and the typed payload; `OutputJson` serializes it compact (no `WriteIndented`), camel-cased, with doubles rounded to a fixed precision and the relaxed encoder so frame names keep their literal `<>&`; the rank builders break weight ties by ordinal name for **deterministic ordering**; golden-file tests pin the byte-exact output (compact JSON is single-line, so the goldens compare cleanly on both OSes). **Deferred within the contract**: the token budget + 25k ceiling enforcement (needs the estimation strategy), the steering-hint *taxonomy* (the channel exists; what each verb nudges toward is decided as the verbs land), and the dense fixed-width text renderer (it adopts the envelope with the M2 CLI head). The symbol-resolution `< 0.8` warning is wired in step 3.
