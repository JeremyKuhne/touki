...
**Progress (2026-06-06):** M0 complete (PR #182). M1 in progress — core relocation merged (PR #183); provider/engine seam laid (`StackSampleSource` + `MetricInfo`); output-contract envelope landed (`AnalysisResult<T>` + compact, rounded, deterministic JSON with golden tests); symbol gate landed (`SymbolGate`; `--strict`/exit-3 deferred to M2); tier-2 LRU landed (`LruCache`); token budget landed (`OutputBudget`; truncation + text renderer deferred to M2); fixture corpus + parity harness landed for the net10 EventPipe half (PR #186); allocation provider landed (`AllocationProvider` reads `GCAllocationTick` into byte-weighted stacks ranked by the same metric-generic engine - `SampleStack.Weight`); GC-stats provider landed (`GcStatsProvider` reads `TraceGC` structured records, reusing the GC-verbose fixture). **ThreadTime is blocked on EventPipe**: a spike showed `SampleProfilerThreadTimeComputer` over a BDN CpuSampling trace reconstructs only `CPU_TIME`/`UNMANAGED_CODE_TIME` (no `BLOCKED_TIME` - blocked threads aren't sampled), so it adds nothing over CPU; real thread-time needs the ETW half or a custom EventPipe session, deferred. Export engine verb landed (`SpeedscopeExporter` writes any provider's stacks to the speedscope sampled format, metric-aware unit). Filter/scope grammar subset landed (`ScopeFilter` include/exclude on frame names; time/process/group deferred for model + fixture reasons). Diff engine verb landed (`RankingDiff` compares two rankings - provider-agnostic regression/improvement deltas). Remaining M1: steering-hint taxonomy + text renderer, tier-1 ETLX disk cache, the deferred filter altitudes + chromium export, `EventQuery`/`JitStats`/`Exceptions` providers, `trim`, O1 spike, the ETW corpus half.
**Date:** 2026-06-04
