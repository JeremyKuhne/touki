...
**Status: in progress.** Step 1 (relocation + provider/engine seam), the output-contract envelope, the symbol gate, the tier-2 LRU cache, and the output token budget have landed, and the parity harness is green for the net10 EventPipe corpus half (§3); the text renderer, new providers/verbs, `trim`, the O1 spike, and the net481 ETW corpus half remain, and the tier-1 ETLX disk cache is deferred to land with the `.etl` fixture (TraceEvent already caches the conversion on both paths).
