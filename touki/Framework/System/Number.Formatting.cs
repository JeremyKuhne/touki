...
internal static partial class Number
{
    ...
    public static void FormatDouble(double value, ref ValueStringBuilder sb, ReadOnlySpan<char> format, NumberFormatInfo info)
    {
        if (FormatDouble(ref sb, value, format, info) is string s)
        {
            sb.Append(s);
        }
    }
    ...
    private static unsafe string? FormatDouble(ref ValueStringBuilder sb, double value, ReadOnlySpan<char> format, NumberFormatInfo info)
    {
        ...
        NumberBuffer number = new(NumberBufferKind.FloatingPoint, pDigits, DoubleNumberBufferLength)
        {
        ...
        if (fmt != 0)
        {
            if (precision == -1)
            {
                Debug.Assert(fmt is 'G' or 'g' or 'R' or 'r');
    ...
    public static void FormatSingle(float value, ref ValueStringBuilder sb, ReadOnlySpan<char> format, NumberFormatInfo info)
    {
        if (FormatSingle(ref sb, value, format, info) is string s)
        {
            sb.Append(s);
        }
    }
        ...
        NumberFormatInfo info)
    {
        ...
        if (thousandSeps)
        {
            ...
            if (info.NumberGroupSeparator.Length > 0)
            {
                ...
                int[] groupDigits = info.GetNumberGroupSizes();
    ...
    private static void FormatCurrency(ref ValueStringBuilder sb, ref NumberBuffer number, int nMaxDigits, NumberFormatInfo info)
    {
        ...
        foreach (char ch in fmt)
        {
            ...
            {
                case '#':
                    FormatFixed(ref sb, ref number, nMaxDigits, info.GetCurrencyGroupSizes(), info.CurrencyDecimalSeparator, info.CurrencyGroupSeparator);
                    break;
                case '-':
                    sb.Append(info.NegativeSign);
                    break;
    ...
    private static void FormatNumber(ref ValueStringBuilder sb, ref NumberBuffer number, int nMaxDigits, NumberFormatInfo info)
    {
        ...
        foreach (char ch in fmt)
        {
            ...
            {
                case '#':
                    FormatFixed(ref sb, ref number, nMaxDigits, info.GetNumberGroupSizes(), info.NumberDecimalSeparator, info.NumberGroupSeparator);
                    break;
                case '-':
                    sb.Append(info.NegativeSign);
                    break;
    ...
    private static void FormatPercent(ref ValueStringBuilder sb, ref NumberBuffer number, int nMaxDigits, NumberFormatInfo info)
    {
        ...
        foreach (char ch in fmt)
        {
            ...
            {
                case '#':
                    FormatFixed(ref sb, ref number, nMaxDigits, info.GetPercentGroupSizes(), info.PercentDecimalSeparator, info.PercentGroupSeparator);
                    break;
                case '-':
                    sb.Append(info.NegativeSign);
                    break;
